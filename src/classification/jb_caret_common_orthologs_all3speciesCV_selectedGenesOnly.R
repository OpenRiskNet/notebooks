...
if(length(args)!=7){
  ...
  print("Please specify locations of: work_dir, human_all_training_and_test_in_single_file, rat_all_training_and_test_in_single_file, 
            mouse_all_training_and_test_in_single_file, orthologs_file, CV_run_count and selected_genes_file ");
  print("Here CV_run_count will start different CV run on a different node, it is an integer between 1 and 10");
  print("The file selected_genes_file contains a single column of only gene names with no header at the top");
  print("Call R with --args option of R");
  print("There must be at least 7 arguments specified. Quitting!");
...
print("all_data size with all genes")
print(dim(all_data))
selected_genes <- read.table(file=selected_genes_file,header=FALSE);
all_data = all_data[,c(1,which(colnames(all_data) %in% as.character(selected_genes[,1])))];
print("Data frame size after only using selected genes");
print("all_data size with selected genes only")
print(dim(all_data))
