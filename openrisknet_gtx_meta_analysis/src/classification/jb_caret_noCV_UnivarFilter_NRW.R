######
# This module is very similar to jb_caret_withSeparateCV_afterUnivarFilter.R, but only difference is that as a test data
# separate file will be provided and that will be used as test data. Test data will not be filtered by univariate filtering.
# And, also there is no cross validation would be done.
######
...
training_data <-read.table(file=training_file, header = TRUE, sep = "\t", row.names = 1)
...
for(i in 2:ncol(training_data)){
  a = anovaScores(training_data[,i],training_data[,1]);
  if(a < signif_cutoff){
    signif_cols <- c(signif_cols,i);
  }
}
print(paste("Number of significant genes with p-value < 0.05: ", length(signif_cols)-1, sep=""));
training_data <- training_data[,signif_cols];

testing_data = read.table(file=validation_data_file, header = TRUE, sep = "\t", row.names = 1)
