...
if(length(args)!=4){
  print(args)
  print("Please specify locations of: work_dir and all_training_and_test_in_single_file and CV_run_count and selected_genes_file with --args option of R");
...
print("Total data frame size");
print(dim(all_data1))

selected_genes <- read.table(file=selected_genes_file,header=FALSE);
all_data = all_data1[,c(1,which(colnames(all_data1) %in% as.character(selected_genes[,1])))];
